...
namespace AiviALAPHAreload
{
    public partial class aiviMainForm : Form
    {
        ...
        SpeechSynthesizer Aivi = new SpeechSynthesizer(); //Creates the speech synthesizer to allow the program to respond
        Grammar shellcommandgrammar; //Grammar variables allow us to load and unload words into Aivi's vocabulary and update them during runtime without the need to restart the program
        ...
        public string sendpath; //send path to MusicPlayer Form
        ...
        {
            InitializeComponent();
            Aivi.Speak("Hello " + Settings.Default.User + ", allow me to load the necessary files");
            string[] defaultcommands = (File.ReadAllLines(Environment.CurrentDirectory + "\\Default Commands.txt")); //Loading all default commands into an array
            foreach (string command in defaultcommands)
            {
                CommandModule.Items.Add(command); //We load them into the listbox on start up just in case someone forgets the commands and they can say, "Show listbox"
            }

            Directory.CreateDirectory(@"C:\Users\" + userName + "\\Documents\\aivi Custom Commands"); //We create 'Aivi Custom Commands' folder in the My Documents folder so we have a place to store our text documents
            Settings.Default.ShellC = @"C:\Users\" + userName + "\\Documents\\aivi Custom Commands\\Shell Commands.txt"; //We save the text document file locations into our settings even before they've been created so we can refer to them easily and globally
            Settings.Default.ShellR = @"C:\Users\" + userName + "\\Documents\\aivi Custom Commands\\Shell Response.txt";
            Settings.Default.ShellL = @"C:\Users\" + userName + "\\Documents\\aivi Custom Commands\\Shell Location.txt";
            Settings.Default.WebC = @"C:\Users\" + userName + "\\Documents\\aivi Custom Commands\\Web Commands.txt";
            Settings.Default.WebR = @"C:\Users\" + userName + "\\Documents\\aivi Custom Commands\\Web Response.txt";
            Settings.Default.WebL = @"C:\Users\" + userName + "\\Documents\\aivi Custom Commands\\Web URL.txt";
            Settings.Default.SocC = @"C:\Users\" + userName + "\\Documents\\aivi Custom Commands\\Social Commands.txt";
            Settings.Default.SocR = @"C:\Users\" + userName + "\\Documents\\aivi Custom Commands\\Social Response.txt";
            ...
            catch { Aivi.SpeakAsync("You may need to restart the program in order to access music files"); } //The try catch block is in case of any unkown errors to prevent the program from crashing.
            ...
            _recognizer.LoadGrammarAsync(new Grammar(new GrammarBuilder(new Choices(File.ReadAllLines(Environment.CurrentDirectory +"\\Default Commands.txt"))))); //Load our Default Commands text document so we have commands to start with
            ...
            startlistening.LoadGrammarAsync(new Grammar(new GrammarBuilder(new Choices("Aivi Come Back Online")))); //Loads a grammar choice into the speech recognition engine
            startlistening.SpeechRecognized += new EventHandler<SpeechRecognizedEventArgs>(startlistening_SpeechRecognized); //The event handler that allows us to say "Aivi Come Back Online". Only one Speech Recognition Engine is active at a time.

            Aivi.SpeakAsync("I am online and ready");
        ...
        void Shell_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            try
            {
                foreach (string line in ArrayShellCommands)
                {
                    if (line == speech) //If line == speech it will open the corresponding program/file
                    {
                        ...
                        Aivi.SpeakAsync(ArrayShellResponse[i]); //Gives the response of the same elemental position as the ArrayShellCommands command that was equal to speech
                    }
            ...
            {
                i += 1;
                Aivi.SpeakAsync("Im sorry it appears the shell command " + speech + " on line " + i + " is accompanied by either a blank line or an incorrect file location");
            }
        }
        ...
        void Social_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            try
            {
                foreach (string line in ArraySocialCommands)
                {
                    if (line == speech)
                    {
                        Aivi.SpeakAsync(ArraySocialResponse[i]);
                    }
                    i += 1;
                }
            }
            ...
            {
                i += 1;
                Aivi.SpeakAsync("Please check the " + speech + " social command on line " + i + ". It appears to be missing a proper response");
            }
        }
        ...
        void Web_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            try
            {
                foreach (string line in ArrayWebCommands)
                {
                    if (line == speech)
                    {
                        ...
                        Aivi.SpeakAsync(ArrayWebResponse[i]);
                    }
                    i += 1;
                }
            }
            ...
            {
                i += 1;
                Aivi.SpeakAsync("Please check the " + speech + "web command on line " + i + ". It appears to be missing a proper response or web U R L");
            }
        }
        ...
        void PlayFile_SpeechRecognized(object sender, SpeechRecognizedEventArgs e)
        {
            ...
            try
            {
                ...
                foreach (string line in FileNames)
                {
                    ...
                    {
                      /*  System.Diagnostics.Process.Start(FileLocations[i]); //touhid
                        Aivi.SpeakAsync(line);
                        QEvent = string.Empty;
                       * */
                        ...
                        mp.Show();
                        sendpath = FileLocations[i];
                        mp.playlist.Items.Add(line);
                       // mp.playlist.Items.Add(FileLocations[i]);
                        //mp.axWindowsMediaPlayer1.Ctlcontrols.play();
                    mp.axWindowsMediaPlayer1.URL = FileLocations[i];
                        mp.axWindowsMediaPlayer1.Ctlcontrols.play();
            ...
            catch { Aivi.SpeakAsync("File names unsuccessfully loaded"); }
        }
        ...
        void ReadDirectories()
        {
            ...
            try
            {
                ...
                if (!File.Exists(@"C:\Users\" + userName + "\\Documents\\Aivi Custom Commands\\Filenames.txt") || QEvent == "ReadDirectories")
                {
                    Aivi.SpeakAsync("I need a few seconds to load audio and video files");
                    sw = File.CreateText(@"C:\Users\" + Environment.UserName + "\\Documents\\Aivi Custom Commands\\Filenames.txt");
                    ...
                    WriteDirectories();
                    sw.Close();
                    Aivi.SpeakAsync("Your libraries have been updated");
                }
                AllLines = File.ReadAllLines(@"C:\Users\" + userName + "\\Documents\\Aivi Custom Commands\\Filenames.txt");
                ...
                catch { Aivi.SpeakAsync("Unable to load files from Music and Video Directories"); }
            }
            catch
            {
                Aivi.SpeakAsync("You do not have permission to access this folder. I will default your libraries back to My Music and My Videos");
                Settings.Default.MusicFolder = Environment.GetFolderPath(Environment.SpecialFolder.MyMusic);
                Settings.Default.VideoFolder = Environment.GetFolderPath(Environment.SpecialFolder.MyVideos);
                Settings.Default.Save();
                sw.Close();
                File.Delete(@"C:\Users\" + Environment.UserName + "\\Documents\\Aivi Custom Commands\\Filenames.txt");
            }
        }
        ...
        private void CommandModule_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            if (CommandModule.SelectionMode == SelectionMode.One)
            {
                if (QEvent == "Play music file")
                {
                    int num = CommandModule.SelectedIndex;
                    try { System.Diagnostics.Process.Start(MyMusicPaths[num]); }
                    catch { Aivi.SpeakAsync("The selected index is out of bounds"); }
                }
        ...
        private void tmrMailCheck_Tick_1(object sender, EventArgs e)
        {
        ...
        private void MusicFBD_HelpRequest(object sender, EventArgs e)
        {
        ...
        private void AlarmTimer_Tick(object sender, EventArgs e)
        {
            ...
            int Ran = rnd.Next(0, arraylength + 1);
            ...
            {
                Settings.Default.AClockEnbl = false; Aivi.SpeakAsync("Initializing alarm sequence");
                System.Diagnostics.Process.Start(MyMusicPaths[Ran]);
            }
        }
        ...
        private void aiviMainForm_MouseDown(object sender, MouseEventArgs e)
        {
        ...
        private void aiviMainForm_MouseMove(object sender, MouseEventArgs e)
        {
            if (Top == 1)
            {
        ...
        private void aiviMainForm_MouseUp(object sender, MouseEventArgs e)
        {
            Top = 0;
        }
        ...
        void WriteDirectories()
        {
            ...
            catch { Aivi.SpeakAsync("Unable to load certain music files"); }
            try
            {
            ...
            catch { Aivi.SpeakAsync("Unable to load certain video files"); }
        }
        ...
        private void aiviMainForm_Load(object sender, EventArgs e)
        {
        ...
        private void button3_Click_1(object sender, EventArgs e)
        {
           ...
           // Console.WriteLine(ExtractTextFromPdf(@"E:\Acadamic\12th Semester\Digital Image Processing\Theory\book\TE_(Day)Final_Summer_2018.pdf"));



            using (PdfReader reader = new PdfReader(@"E:\Acadamic\12th Semester\Digital Image Processing\Theory\book\TE_(Day)Final_Summer_2018.pdf"))
            {
